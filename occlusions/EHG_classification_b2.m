...
for k = 1:length(headerFiles)
    ...
    [s,f] = ((stft(downsample_signal(:,1),fs,'Window',hamming(window_length),'Centered',0)));
    f_in_half = flip(f(round(size(s,1)/2):end)-5);

    [~,f_2_2] = min(abs(f_in_half-2.2));
    [~,f_3_5] = min(abs(f_in_half-3.5));
    ...
    s_half{count} = s_half{count}(f_3_5:f_2_2,:);
    count = count+1;
...
save('./occlusions_2/results_EHG_B2','y_hat','y_hat_2','y_test_stored');
