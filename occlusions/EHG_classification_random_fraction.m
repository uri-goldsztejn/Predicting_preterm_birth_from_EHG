...
for k = 1:length(headerFiles)
    ...
    time_length = 106;
    fraction = 1.0;
    affected_idx = randperm(time_length);
    affected_idx = affected_idx(1:round(time_length*fraction));
    affected_idx_reorder = randperm(length(affected_idx));

    s_half{count}(:,affected_idx) = s_half{count}(:,affected_idx(affected_idx_reorder));
...
file_name = ['./occlusions_2/time_shuffle_2/EHG_results_rand_', num2str(fraction*100)];
save(file_name,'y_hat','y_hat_2','y_test_stored');
